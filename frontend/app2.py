...
def parse_lol_data(data):
    ...
    for event in data['data']['esports']['events']:
        if event['__typename'] == 'EventMatch' and 'match' in event:
            ...
            # Game info
            games_completed = sum(1 for game in match.get('games', []) if game.get('state') == 'completed')
            ...
            if any(winners):
                ...
                winner = teams[winner_index] if winner_index >= 0 and winner_index < len(teams) else ""
...
if not show_current:
        today_start = datetime.combine(current_date.date(), datetime.min.time(), tzinfo=pytz.timezone('Europe/Andorra'))
        today_end = datetime.combine(current_date.date(), datetime.max.time(), tzinfo=pytz.timezone('Europe/Andorra'))
        filtered_df = filtered_df[~((filtered_df['start_time'] >= today_start) & (filtered_df['start_time'] <= today_end))]
if not show_future:
    filtered_df = filtered_df[filtered_df['start_time'] <= current_date]
                ...
                with st.container():
                    ...
                    with cols[2]:
                        ...
                        if row.state == 'completed':
                            ...
                            st.write(f"**Played matches:** {row.games_completed}/{row.total_games}")
                        else:
                    ...
                    with cols[3]:
                        ...
                        if row.match_id in st.session_state.predictions:
                            ...
                            if "error" in prediction:
                                ...
                                st.success(f"Win probability: {prediction.get('pred', 0):.1%}")
